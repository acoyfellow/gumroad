...
describe CustomerPresenter do
  ...
  describe "#missed_posts" do
    ...
    it "passes workflow_id to missed_for_purchase when provided" do
      ...
      expect(Installment).to receive(:missed_for_purchase).with(purchase, workflow_id: workflow_id).and_call_original
  ...
  describe "#customer_emails" do
    ...
    it "returns correct props for customer_emails while including only sent posts for the purchase's product, excludes other product posts" do
      create(:creator_contacting_customers_email_info_delivered, installment: regular_post_product_a, purchase:)
      create(:creator_contacting_customers_email_info_delivered, installment: regular_post_product_b, purchase:)

      result = described_class.new(purchase:).customer_emails
      receipts = result.select { |e| e[:type] == "receipt" }
      posts = result.select { |e| e[:type] == "post" }

      expect(receipts.size).to eq(1)
      expect(receipts.first).to include(type: "receipt", id: purchase.external_id, name: "Receipt", state: "Delivered")
      expect(receipts.first).to have_key(:url)
      expect(receipts.first).to have_key(:state_at)
      expect(receipts.first).to have_key(:date)

      expect(posts.size).to eq(1)
      expect(posts.first).to include(type: "post", id: regular_post_product_a.external_id, name: regular_post_product_a.name, state: "Delivered")
      expect(posts.first).to have_key(:state_at)
      expect(posts.first).to have_key(:date)
    end

    it "returns only the latest email per installment when multiple emails exist" do
      create(:creator_contacting_customers_email_info_delivered, installment: regular_post_product_a, purchase:)
      create(:creator_contacting_customers_email_info_delivered, installment: regular_post_product_a, purchase:, sent_at: 1.day.ago)
      create(:creator_contacting_customers_email_info_delivered, installment: workflow_post_product_a, purchase:)
      create(:creator_contacting_customers_email_info_delivered, installment: workflow_post_product_a, purchase:, sent_at: 1.day.ago)

      result = described_class.new(purchase:).customer_emails
      receipts = result.select { |e| e[:type] == "receipt" }
      post_entries = result.select { |e| e[:type] == "post" }

      expect(receipts.size).to eq(1)
      expect(receipts.first).to include(type: "receipt", id: purchase.external_id)
      expect(receipts.first).to have_key(:url)
      expect(receipts.first).to have_key(:state_at)
      expect(receipts.first).to have_key(:date)

      expect(post_entries.length).to eq(2)
      expect(post_entries.map { |e| e[:id] }).to eq([workflow_post_product_a.external_id, regular_post_product_a.external_id])
      post_entries.each do |entry|
        expect(entry).to include(type: "post", state: "Delivered")
        expect(entry).to have_key(:name)
        expect(entry).to have_key(:id)
        expect(entry).to have_key(:state_at)
