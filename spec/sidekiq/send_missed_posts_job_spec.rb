...
describe SendMissedPostsJob do
  ...
  describe "#perform" do
    ...
    it "raises CustomerDNDEnabledError when perform is called directly" do
      ...
      expect do
        described_class.new.perform(purchase.id)
      end.to raise_error(SendPostsForPurchaseService::CustomerDNDEnabledError, customer_dnd_enabled_error_message)
    end
    ...
    it "raises SellerNotEligibleError when perform is called directly" do
      ...
      expect do
        described_class.new.perform(purchase.id)
      end.to raise_error(SendPostsForPurchaseService::SellerNotEligibleError, seller_not_eligible_error_message)
    end
  end

  # NOTE: The sidekiq_retry_in callback that discards the job on
  # SendPostsForPurchaseService::CustomerDNDEnabledError and SellerNotEligibleError exceptions
  ...
  describe "sidekiq_retry_in callback" do
    ...
    it "returns :discard and logs for CustomerDNDEnabledError" do
      exception = SendPostsForPurchaseService::CustomerDNDEnabledError.new(customer_dnd_enabled_error_message)
    ...
    it "returns :discard and logs for SellerNotEligibleError" do
      exception = SendPostsForPurchaseService::SellerNotEligibleError.new(seller_not_eligible_error_message)
