...
describe("Discover", js: true, type: :system) do
  ...
  describe "offer codes search feature" do
    let!(:black_friday_product) do
      create(:product, :recommendable, user: create(:compliant_user, name: "Black Friday Seller"),
                                       name: "Black Friday Deal", taxonomy: three_d_taxonomy)
    end
    ...
    context "when feature is enabled for the user" do
      ...
      it "shows black friday hero and filters products by offer code" do
        visit discover_url(host: discover_host, offer_code: SearchProducts::BLACK_FRIDAY_CODE)
        wait_for_ajax

        # Black Friday hero should be visible
        expect(page).to have_text("Get Black Friday deals")

        # Products should be filtered by offer code
        expect(page).to have_current_path("/?offer_code=#{SearchProducts::BLACK_FRIDAY_CODE}")

        # Recommended products section should not be present
        expect(page).not_to have_text("Featured products")
      end
    end
    ...
    context "when feature is disabled for the user" do
      ...
      it "does not show black friday hero" do
        visit discover_url(host: discover_host, offer_code: SearchProducts::BLACK_FRIDAY_CODE)
        wait_for_ajax

        # Black Friday hero should not be visible
        expect(page).not_to have_text("Get Black Friday deals")

        # Should show normal discover page without filtering
        expect(page).to have_text("Featured products")
      end
    end
    ...
    context "when feature is enabled via feature_key" do
      ...
      it "shows black friday hero with valid feature_key and hides recommended products" do
        visit discover_url(host: discover_host, offer_code: SearchProducts::BLACK_FRIDAY_CODE, feature_key: "secret123")
        wait_for_ajax
    ...
    context "when logged out" do
      it "respects global feature flag setting" do
        # With valid offer code, should show black friday hero if feature is enabled
        ...
        # With feature disabled, should not show black friday hero
        Feature.deactivate(:offer_codes_search)
        page.refresh

        expect(page).not_to have_text("Get Black Friday deals")

        # With invalid offer code, should show normal discover page
        Feature.activate(:offer_codes_search)
        visit discover_url(host: discover_host, offer_code: "INVALID_CODE")
        wait_for_ajax
