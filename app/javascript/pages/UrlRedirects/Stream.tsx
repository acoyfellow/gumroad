...
type PlaylistItem = StreamPageProps["playlist"][number];

const clonePlaylistWithMutableLocations = (playlist: PlaylistItem[]): PlaylistItem[] =>
  playlist.map((item) => ({
    ...item,
    latest_media_location: item.latest_media_location == null ? null : { ...item.latest_media_location },
  }));
...
const StreamPage = () => {
  ...
  } = cast<StreamPageProps>(usePage().props);
  ...
  useRunOnce(() => {
    const createPlayer = async () => {
      ...
      const playlist = clonePlaylistWithMutableLocations(initialPlaylist);
