...
class Products::Edit::ProductController < Products::Edit::BaseController
  def show
    ...
    render inertia: "Products/Edit/Product", props: @presenter.edit_props
  end
  ...
  def update
    begin
       ActiveRecord::Base.transaction do
        ...
        return redirect_to products_edit_product_path(@product.unique_permalink), alert: error_message
     end
    ...
    if all_invalid_offer_codes.any?
      ...
      return redirect_to products_edit_product_path(@product.unique_permalink), warning: "The following offer #{"code".pluralize(all_invalid_offer_codes.count)} #{issue_description}: #{all_invalid_offer_codes.join(", ")}. Please update #{all_invalid_offer_codes.length > 1 ? "them or they" : "it or it"} will not work at checkout."
    end
