...
class UrlRedirectsController < ApplicationController
  ...
  layout "inertia", only: [:confirm_page, :stream]
  ...
  before_action :hide_layouts, only: %i[
    membership_inactive_page expired rental_expired_page show download_page download_product_files smil hls_playlist download_subtitle_file read
  ]
  ...
  def stream
    ...
    render inertia: "UrlRedirects/Stream", props: presenter.stream_page_props(product_file: @product_file)
  end
